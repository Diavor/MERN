...
const createPage = (body) =>
  request(app).post("/api/pages").set("Authorization", `Bearer ${adminToken}`).send(body);
...
describe("CMS page visibility", () => {
  test("a published public page is served by slug", async () => {
    await createPage({ title: "Storia", slug: "storia", status: "published", visibility: "public" });
  ...
  test("a draft page is not served publicly", async () => {
    await createPage({ title: "Bozza", slug: "bozza", status: "draft", visibility: "public" });
  ...
  test("a private published page is hidden from the storefront", async () => {
    await createPage({ title: "Riservata", slug: "riservata", status: "published", visibility: "private" });
  ...
  test("a scheduled page is hidden until its publishDate has passed", async () => {
    ...
    await createPage({ title: "Futura", slug: "futura", status: "published", visibility: "scheduled", publishDate: future });
    const hidden = await request(app).get("/api/pages/slug/futura");
    assert.equal(hidden.status, 404);

    const past = new Date(Date.now() - 86400000).toISOString();
    await createPage({ title: "Passata", slug: "passata", status: "published", visibility: "scheduled", publishDate: past });
